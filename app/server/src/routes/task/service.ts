...
export const taskService = {
    ...
    async createTask(fastify: any, data: createTaskDTO, userId: number): Promise<taskResponseDTO> {
        ...
        try {
            ...
            const newTask = await client.query(
                ...
                [data.taskName, nextIndex, data.boardColumnId, data.description || '', startingDate, userId] // Utilisation du vrai userId
            );
    ...
    async getTasks(fastify: any, boardColumnId: number): Promise<taskResponseDTO> {
        try {
            // CORRECTION SQL : UserInfo au lieu de User, et u.name au lieu de u.username
